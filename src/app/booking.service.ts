...
export class BookingService {
  ...
  private bookingsSubject = new BehaviorSubject<Booking[]>(this.bookingsData);
  ...
  refreshBookings(): void {
    this.bookingsSubject.next([...this.bookingsData]);

    // this.http.get('https://www.capilog.fr/outils/resa/resa.php', { responseType: 'text' }).subscribe({
    //   next: (html) => {
    //     try {
    //       const match = html.match(/<pre[^>]*>(.*?)<\/pre>/s);
    //       if (match && match[1]) {
    //         const jsonText = match[1].trim();
    //         const bookings: Booking[] = JSON.parse(jsonText);
    //         if (bookings.length === 0) {
    //           console.warn('Aucun booking');
    //         }
